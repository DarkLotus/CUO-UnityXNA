...
namespace ClassicUO.Renderer
{
    ...
    public class SpriteBatch3D
    {
        ...
        private UnityEngine.Rect _drawingArea;
        ...
        public SpriteBatch3D(GraphicsDevice device)
        {
            ...
            _effect = new Effect( GraphicsDevice );// new Effect(GraphicsDevice, File.ReadAllBytes(Path.Combine(Bootstrap.ExeDirectory, "shaders/IsometricWorld.fxc")));
        ...
        public unsafe bool DrawSprite(Texture2D texture, SpriteVertex[] vertices, Techniques technique = Techniques.Default, Rectangle? scissorRectangle = null)
        {
            ...
            for (byte i = 0; i < 4; i++)
            {
                if (_drawingArea.Contains(vertices[i].Pos) == true)
                {
                    draw = true;
